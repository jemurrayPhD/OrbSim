...
def normalize_quantum_numbers(symbol: str, n: int | None, l: int | None, m: int) -> tuple[int, int, int]:
    """Clamp quantum numbers to allowed ranges, falling back to element defaults."""
...
def evaluate_orbital_values(
    symbol: str, n: int | None, l: int | None, m: int, coordinates: np.ndarray
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Evaluate psi, amplitude, probability, and phase at given Cartesian coordinates (angstrom)."""
    coords = np.asarray(coordinates, dtype=float).reshape(-1, 3)
    n_val, l_val, m_val = normalize_quantum_numbers(symbol, n, l, m)
    r_cart = np.linalg.norm(coords, axis=1)
    cos_theta = np.clip(coords[:, 2] / (r_cart + 1e-9), -1.0, 1.0)
    ...
    return psi, amplitude, probability, phase
...
def field_from_grid(
    ...
    """Convert a sampled grid into an OrbitalField with surface/volume representation."""
    ...
    if representation == "volume":
        opacity_values = np.linspace(0.05, 0.95, 256)
    ...
    probability = np.asarray(grid.get_array("probability"))
    ...
    if surface.n_points == 0:
        span = max(grid.spacing[0] * grid.dimensions[0], grid.spacing[1] * grid.dimensions[1], grid.spacing[2] * grid.dimensions[2]) * 0.1
        surface = pv.Sphere(radius=span if span > 0 else 1.0)
        surface[scalar_name] = np.zeros(surface.n_points)
    surface.set_active_scalars(scalar_name)

    mask_prob = probability >= iso_value if probability.size else np.array([], dtype=bool)
    cumulative = 0.0
    voxel_volume = float(np.prod(grid.spacing))
    total_prob = float(probability.sum() * voxel_volume) if probability.size else 0.0
    if total_prob > 0 and mask_prob.size:
    ...
    return OrbitalField(
        ...
        cumulative_probability=cumulative if total_prob > 0 else None,
    )
...
def make_orbital_mesh(
    ...
    psi, amplitude, probability, phase = evaluate_orbital_values(symbol, n_val, l_val, m_val, coords)
    ...
    return field_from_grid(grid, mode, representation=representation, iso_fraction=iso_fraction)
