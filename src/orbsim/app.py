...
from orbsim.orbitals import default_quantum_numbers, make_orbital_mesh, occupied_orbitals, _ATOMIC_NUMBER
...
class AtomicOrbitalTab(QtWidgets.QWidget):
    ...
    def _build_controls(self) -> QtWidgets.QWidget:
        ...
        self.slice_cmap_combo.addItems(["viridis", "plasma", "inferno", "twilight", "coolwarm", "twilight_shifted"])
    ...
    def _iso_color_for_fraction(self, frac: float) -> str:
        ...
        cmap = cm.get_cmap(self.current_cmap)
        r, g, b, _ = cmap(frac_clamped)
    ...
    def _render_slice_colorbar(self, cmap: str, clim: tuple[float, float], label: str) -> None:
        ...
        try:
            ...
            fig, ax = plt.subplots(figsize=(4.4, 0.6))
            fig.patch.set_facecolor("#0f172a")
            cbar = fig.colorbar(sm, cax=ax, orientation="horizontal")
            cbar.set_label(label, color="#e5e7eb", fontsize=9)
            ...
            self.slice_colorbar.setMinimumHeight(pixmap.height() + 12)
    ...
    def _apply_slice(self, fields, volume_field, clim: tuple[float, float]) -> None:
        ...
        try:
            if self.pref_show_slice_plane:
                ...
                self.slice_plane_actor = self.plotter.add_mesh(
                    plane_geom,
                    color="#cbd5e1",
                    opacity=float(self.pref_slice_plane_opacity),
                    lighting=False,
                    show_scalar_bar=False,
                    style="wireframe",
                )
        ...
        try:
            ...
            try:
                self._slice_data_range = (float(np.nanmin(scalar_data)), float(np.nanmax(scalar_data)))
            except Exception:
